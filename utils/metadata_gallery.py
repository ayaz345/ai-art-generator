...
class PromptFile:
    def __init__(self, prompt_dir, prompt_file, opt):
        ...
        if opt.o_use_upscaler != "":
            if opt.o_use_upscaler.lower() in ["no", "yes"]:
        ...
        if opt.o_upscaler_face_enh != "":
            if opt.o_upscaler_face_enh.lower() in ["no", "yes"]:
    ...
    def init_prompts(self):
        ...
        self.write(f"# python make_art.py prompts/generated/{self.prompt_file_name}")
        ...
        if self.opt_overrides:
            ...
            if (self.width != "0") and (self.height != "0"):
                self.write(f"!WIDTH = {self.width}")
                self.write(f"!HEIGHT = {self.height}")

            if (self.steps > 0):
                self.write(f"!STEPS = {str(self.steps)}")

            if (self.scale > 0):
                self.write(f"!SCALE = {str(self.scale)}")

            if self.use_upscale in ["yes", "no"]:
                self.write(f"!USE_UPSCALE = {self.use_upscale}")

            if (self.upscale_amount > 0):
                self.write(f"!UPSCALE_AMOUNT = {str(self.upscale_amount)}")

            if self.upscale_face_enh in ["yes", "no"]:
                self.write(f"!UPSCALE_FACE_ENH = {self.upscale_face_enh}")
    ...
    def add_prompt_section(self, prompt, width, height, steps, scale, initimg, initstr, upscale_use, upscale_amt, upscale_face):
        ...
        if (self.width == 0) and (self.height == 0):
            self.write(f"!WIDTH = {str(width)}")
            self.write(f"!HEIGHT = {str(height)}")
            wrote_opt = True

        if (self.steps == 0):
            self.write(f"!STEPS = {str(steps)}")
            wrote_opt = True

        if (self.scale == 0):
            self.write(f"!SCALE = {str(scale)}")
            wrote_opt = True

        if initimg != "":
            wrote_opt = True
            if not self.ignore_input_images:
                self.write(f"!INPUT_IMAGE = {initimg}")
                self.write(f"!STRENGTH = {str(initstr)}")
            else:
        ...
        if self.use_upscale == "":
            wrote_opt = True
            self.write(f"!USE_UPSCALE = {upscale_use}")

        if upscale_use == "yes" or self.use_upscale == "yes":
            if (self.upscale_amount == 0):
                wrote_opt = True
                self.write(f"!UPSCALE_AMOUNT = {str(upscale_amt)}")
            if self.upscale_face_enh == "":
                wrote_opt = True
                self.write(f"!UPSCALE_FACE_ENH = {upscale_face}")
    ...
    def cleanup(self):
        ...
        self.f.close()
        print(f"Created prompt file as {self.prompt_file_dir}{self.prompt_file_name}")
...
class Html:
    ...
    def cleanup(self, exec_time):
        footer_text = (
            f"Generated in {str(exec_time)} milliseconds on {str(date.today())} at "
            + time.strftime("%H:%M:%S")
        )
        self.write("</div>")
        ...
        self.write("</html>")
        self.f.close()
        print(f"Created gallery file as {self.html_file_name}")
